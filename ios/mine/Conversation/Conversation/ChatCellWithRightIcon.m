//
//  ChatCellWithRightIcon.m
//  Conversation
//
//  Created by Xinjun Email on 22/07/12.
...
@implementation ChatCellWithRightIcon
...
-(void) fillContent:(MessageObject*)message IconImage:(UIImage*) image
{
    ...
    self.chatMessageView.frame = CGRectMake(CHATVIEW_DEFAULT_IMAGE_MESSAGE_GAP, 0, self.chatMessageView.bounds.size.width, self.fromWhomLabel.bounds.size.height+height+CHATVIEW_DEFAULT_MESSAGE_ADDRESS_GAP+self.sentFromWhereLabel.bounds.size.height+CHATVIEW_DEFAULT_ADDRESS_BOTTOM_GAP); 

    self.msgLabel.frame = CGRectMake(self.msgLabel.frame.origin.x, self.fromWhomLabel.bounds.size.height, self.msgLabel.frame.size.width, height);
    ...
    bubbleImage = [[UIImage imageNamed:@"ChatBubbleGreen.png"] stretchableImageWithLeftCapWidth:23 topCapHeight:15];
    
    msgBackground.image = bubbleImage;
    
    //recalculated the origin height 
    self.iconImageView.frame = CGRectMake(CHATVIEW_DEFAULT_IMAGE_MESSAGE_GAP+self.chatMessageView.bounds.size.width, height+self.fromWhomLabel.bounds.size.height+CHATVIEW_DEFAULT_MESSAGE_ADDRESS_GAP+self.sentFromWhereLabel.bounds.size.height-self.iconImageView.bounds.size.height, self.iconImageView.frame.size.width, self.iconImageView.frame.size.height);
    self.iconImageView.image = image; 
    
    //add a black border for the image
    [self.iconImageView.layer setBorderColor: [[UIColor greenColor] CGColor]];
